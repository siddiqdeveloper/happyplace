...
export class ProductImageComponent implements OnInit {
  ...
  listAllProducts() {
    ...
    this.loading = true;
    this.apiService
      .getData("productDetails", this.route.snapshot.paramMap.get("id"))
      .subscribe(
        (data) => {
  ...
  convertFile(index) {
    ...
    if (this.productDetails.product_type == "dp") {
      var blob = this.files[0];
      console.log();
      let tfile: any = this.files[0].fileEntry;

      const file = tfile;
      tfile.file((blob: File) => {
        console.log(blob);
  ...
  drop() {
    ...
    this.apiService
      .imageosorting({ data: this.allImageList })
      .subscribe((res) => {
