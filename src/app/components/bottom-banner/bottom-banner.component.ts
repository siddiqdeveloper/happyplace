...
export class BottomBannerComponent implements OnInit {
  ...
  index() {
      this.loading = true;
      this.apiService.index('bottomBanner').subscribe(data => {
  ...
  store() {
      ...
      this.apiCall('store', 'bottomBanner');
  }

  show(id) {
      this.apiService.show('bottomBanner/' + id).subscribe((data) => {
  ...
  bannerSwitch(id) {
      this.apiCall('getData', 'bottomBannerSwitch/' + id);
  }
  ...
  swalCall(id) {
      Swal.fire({
          ...
          if (result.value) {
              this.apiCall('destroy', 'bottomBanner/' + id);
              Swal.fire(
                  'Deleted!',
