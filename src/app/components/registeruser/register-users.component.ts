...
export class RegisterUsersComponent implements OnInit {
  ...
  listAllOrders() {
    this.loading = true;
    this.apiService.getData("regsterUserList").subscribe(
      (data) => {
  ...
  orderStatusUpdate(id) {
    ...
    this.apiService
      .getData("orderStatusUpdate", `${id}/${this.currentStatus}`)
      .subscribe((data) => {
        if (data.error === false) {
          this.listAllOrders();
  ...
  searchOrder() {
    this.searchField.valueChanges
      .pipe(
        ...
        switchMap((query) => this.apiService.searchData("searchOrder", query))
      )
      .subscribe((result) => {
