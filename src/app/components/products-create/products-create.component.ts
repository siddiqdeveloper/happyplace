...
export class ProductsCreateComponent implements OnInit {
  ...
  subcategory_id: any;
    ...
    constructor(public router:Router,public ar:ActivatedRoute,  private apiService: ApiService, private toastr: ToastrService, private imageService: ImageService, private formBuilder: FormBuilder) { }
   ...
   const abc = this.subcategory.filter(sub => {
    ...
    apiCall(name, url, value: any = '') {
        ...
        if(this.sizeArray.length>0){
            this.selectedSize = this.sizeArray[0].id;
          }

      if(this.colorArray.length>0){
        this.selectedColor = this.colorArray[0].id;
      }
